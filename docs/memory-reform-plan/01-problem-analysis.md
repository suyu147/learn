...
**位置**: `lib/deeptutor/bootstrap.ts:252-255`
...
`write_memory` 和 `read_memory` 工具的 `_userId` 在 bootstrap 时设为 `'anonymous'`，之后**没有任何代码更新它**。即使用户已登录、turns route 拿到了真实 userId，工具仍然使用 `'anonymous'`。

**影响**: 所有记忆写入 `data/memory/anonymous/`，而非真实用户目录。
...
1. **系统提示词中没有引导 LLM 使用记忆工具**。`chat-capability.ts:101-107` 调用 `assembleSystemPrompt` 时，`memoryContext` 参数来自 `context.memoryContext`，而 turns route 中的 `createUnifiedContext` **没有设置 `memoryContext`**。
...
`lib/deeptutor/core/orchestrator.ts` 会 emit `CAPABILITY_COMPLETE`，`memory-subscriber.ts` 也确实监听了。但 consolidate 的前提是 L1 有 trace 事件——而 L1 trace 只在 `write_memory` 被调用时写入。如果 `write_memory` 从未被调用（断点 2），L1 就是空的，consolidate 无数据可处理。
...
| **userId** | PathService 按用户隔离 | 硬编码 'anonymous' |
...
**加剧因素**:
1. userId 硬编码为 'anonymous'，即使有数据也存错位置
