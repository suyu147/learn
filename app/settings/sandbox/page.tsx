    ...
    <div className="p-6 max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-[13px] text-[var(--muted-foreground)]">
          Configure the Piston code execution engine, supported languages, and resource limits.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[13px] font-medium text-[var(--foreground)]">API Endpoint URL</label>
            <input
              type="text"
              value={pistonUrl}
            ...
            <p className="text-[11px] text-[var(--muted-foreground)]">
              URL of your Piston instance. Deploy locally or use a remote server.
            </p>
          </div>

          <div className="space-y-2">
            <label className="text-[13px] font-medium text-[var(--foreground)]">API Key (optional)</label>
            <input
              type="password"
              value={pistonApiKey}
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[13px] font-semibold text-[var(--foreground)]">
              Language Support ({enabledCount}/{languages.length} enabled)
            </h3>
          </div>
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[13px] font-medium text-[var(--foreground)] flex items-center gap-1.5">
              ...
              Execution Timeout: {executionTimeout}s
            </label>
            <input
              type="range"
              min={5}
          ...
          <div className="space-y-2">
            <label className="text-[13px] font-medium text-[var(--foreground)]">
              Daily Execution Quota: {dailyQuota} runs
            </label>
            <input
              type="range"
              min={10}
        ...
        <div className="space-y-3 pt-2">
          <ToggleField
            ...
            description="Permit code execution to make outbound HTTP requests (security risk)"
          />
          <ToggleField
            checked={captureStderr}
            onChange={setCaptureStderr}
            label="Capture Stderr"
            description="Include standard error output alongside stdout in execution results"
          />
          <ToggleField
            checked={logExecutions}
            onChange={setLogExecutions}
            label="Log All Executions"
            description="Record every code execution event for debugging and audit purposes"
          />
        </div>
      </div>
