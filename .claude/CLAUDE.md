...
- **39 个** `/api/v1/*` 路由已在工作树中创建
- **17 个**旧路由（`/api/learn`、`/api/chat`、`/api/tutor/chat` 等）已在工作树中删除

### ⚠️ 紧急风险：v1 路由迁移未提交

所有 v1 路由变更**只存在于工作树中，未提交到 Git**。如果执行 `git checkout` 或 `git stash`，39 个 v1 路由会消失、17 个旧路由会恢复。

**建议**：立即提交当前工作树的 API 路由迁移变更。
...
项目根目录有 3 个 TS 错误日志（`tsc-errors.txt`、`tsc-errors2.txt`、`tsc-errors3.txt`），记录了：
- `lib/action/engine.ts` — 模块解析失败、Action 类型不兼容
- `components/chat/use-chat-sessions.ts` — 类型错误

**要求**：在开始 Phase 0 之前，先确保 `npm run build` 能通过。

### 4.2 所有页面使用硬编码 Mock 数据

/chat、/smartlearn、/book、/co-writer、/knowledge、/memory、/profile 等所有页面使用组件内硬编码 mock 数据，未通过 API 获取真实数据。前端视觉上完整，但后端未打通。

### 4.3 Prisma Schema 未接入

8 个 Prisma 模型定义了但从未被调用（详见第二节）。

### 4.4 Store 双重体系

新旧 store 并存（详见第二节），需要统一。
...
| 9 | 统一 `/api/v1/*` | 17 个旧路由已全部迁移（工作树中，待提交） |
